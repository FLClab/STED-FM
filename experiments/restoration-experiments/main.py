    ...
    for i, (X, y) in enumerate(tqdm(valid_loader, desc="[----] ")):
        ...
        if (i == 0) and args.use_tensorboard:
            ...
            for i in range(cfg.dataset_cfg.num_classes):
                writer.add_images(f"Images-test/label-{i}", intensity_scale_(y[:16, i:i+1]), epoch, dataformats="NCHW")
                writer.add_images(f"Images-test/pred-{i}", intensity_scale_(pred[:16, i:i+1]), epoch, dataformats="NCHW")
...
if __name__ == "__main__":
    ...
    # Count model parameters and number of trainable parameters
    ...
    for epoch in range(start_epoch, cfg.num_epochs):
        ...
        for i, (X, y) in enumerate(tqdm(train_loader, desc="[----] ")):
            ...
            if (i == 0) and args.use_tensorboard:
                ...
                for i in range(cfg.dataset_cfg.num_classes):
                    writer.add_images(f"Images-train/label-{i}", intensity_scale_(y[:16, i:i+1]), epoch, dataformats="NCHW")
                    writer.add_images(f"Images-train/pred-{i}", intensity_scale_(pred[:16, i:i+1]), epoch, dataformats="NCHW")
