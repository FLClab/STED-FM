...
def filter_spots(mask):
        ...
        """
        img = np.copy(mask).astype(bool)
        
        img = morphology.remove_small_objects(img, min_size=30)
...
class DetectionWavelets:
    ...
    def computeDetection(self):
        ...
        :return image_out: np array representing the binary image
        """

        data_in = np.copy(self.img).astype('float32')
    ...
    def b3WaveletCoefficients2D(self, scale_coefficients, original_image):
        ...
        :param scale_coefficients: List of  convoluted images as 2D np arrays
        :param original_image: Original image as 1D list
        :return wavelet_coefficients: List of coefficient images 2D np arrays
        """
        ...
        wavelet_coefficients = np.stack(wavelet_coefficients, 0)
    ...
    def filter_wat(self, data, depth):
        ...
        for i in range(self.J + 2):
            lambdac.append(np.sqrt(2 * np.log(data.size / (1 << (2 * i)))))
    ...
    def spot_construction(self, input_coefficients):
        ...
        :param input_coefficients: 3D np of array wavelet coefficient images
        :return output: Correlation image as 2D np array
        """
        J_array = np.array(self.J_list)-1
        #zero_coords = np.prod(input_coefficients[J_array], axis=0) > 0

        output = np.prod(input_coefficients[J_array], axis=0)
        #output *= zero_coords
        return output
    ...
    def filter_and_swap(array_in, stepS):
        ...
        for s in range(stepS):
            if s > 0:
                kernel = np.insert(kernel, inter, 0)
                inter = inter + np.array([1, 2, 3, 4])
        kernel_base = np.zeros((kernel.size, kernel.size))
...
class SpatialDistribution:
    ...
    def __init__(self, prob_map, img, cs=0, min_axis=1):
        ...
        :param prob_map: A 2D np array of boolean detected clusters
        :param img: np array, image data used for intensity weighting for centroid
    ...
    def poly_area(self, x, y):
        ...
        :param x: A np array of x coordinates
        :param y: A np array of y coordinates
        """
...
class SpatialRelations:
    ...
    def __init__(self, MPP1, MPP2, sROI, roivolume, poly, img, n_rings, step, filename):
        ...
        :param img: np array containing image data
        ...
        self.rings = np.array([r*step for r in range(0, n_rings+1, 1)])
    ...
    def matrix_dist(X, Y):
        ...
        :param X: np array with shape (len(MPP1), 2), coordinates of every points in channel 1
        :param Y: np array with shape (len(MPP2), 2), coordinates of every points in channel 2
        :return: np array of distances between every combination of points in the two channels
        """
    ...
    def nearest_neighbors(self):
        ...
        mpp1_data = np.zeros((len(self.MPP1_ROI), 2))
        mpp2_data = np.zeros((len(self.MPP2_ROI), 2))
        ...
        min_distance = []
        for x in mpp1_data:
            repmat = np.tile(x, (mpp1_data.shape[0], 1))
            distance = self.matrix_dist(repmat, mpp1_data)
            distance_pos = distance[np.where((distance > 0))]
            distances.append(distance)
            m = distance_pos.min()
            m_index = np.where((distance == m))[0][0]
        ...
        min_distance = []
        for x in mpp1_data:
            repmat = np.tile(x, (mpp2_data.shape[0], 1))
    ...
    def correlation_new(self):
        ...
        distances^2
        :return result: np array in which the first row is Ripley's G matrix
        """
        result = np.zeros((3, self.N_fit - 1))
        delta_K = np.zeros(self.N_fit - 1)
        d_mean = np.zeros(self.N_fit - 1)
        d_mean_2 = np.zeros(self.N_fit - 1)
        count = np.zeros(self.N_fit - 1)
        ROIy, ROIx = self.sROI
                        ...
                        for l in range(max(j - 1, 0), min(j + 1, len(self.imgw1[i]) - 1) + 1):
                            for y2, x2, s2, db2 in self.imgw2[k][l]:
                                ...
                                if (y1, x1, s1) != (y2, x2, s2):
                                    weight = 1 # weight
                                    if temp > d:
                                        weight = 1 - (np.arccos(d / temp)) / np.pi
    ...
    def nearest_contour(self, mpp):
        ...
        """
        mpp_data = np.zeros((len(mpp), 2))

        poly_sum = np.concatenate(self.poly)

        poly_data = np.zeros((len(poly_sum), 2))
        ...
        min_distance = []
        for x in mpp_data:
            repmat = np.tile(x, (poly_data.shape[0], 1))
    ...
    def variance_theo_delta_new(self):
        ...
        :return result: np array, variance matrix
        """
        n1, n2 = len(self.MPP1_ROI), len(self.MPP2_ROI)
        mu = self.mean_G()
        result = np.zeros(self.N_fit - 1)
        ...
        for k in range(1, self.N_fit):
            ...
            d2 = distancek_1**2
            d2bis = distancek**2
            e1 = np.pi * (distancek_1**2)
            e2 = np.pi * (distancek**2)
                                ...
                                for y2, x2, s2, db2 in self.imgw1[m][l]:
                                    ...
                                    if distance_ij > 0:
                                        if distance_ij < 2 * distancek_1:
                                            temp1 = 2 * d2 * np.arccos(distance_ij / (2 * distancek_1))
                                            temp2 = 0.5 * distance_ij * np.sqrt(4 * d2 - distance_ij**2)
                                            temp_A1 += temp1 - temp2
                                        if distance_ij < 2 * distancek:
                                            temp1 = 2 * distancek**2 * np.arccos(distance_ij / (2 * distancek))
                                            temp2 = 0.5 * distance_ij * np.sqrt(4 * distancek**2 - distance_ij**2)
                                            temp_A2 += temp1 - temp2
                                        if distance_ij < distancek_1 + distancek:
                                            if distance_ij + distancek_1 < distancek:
                                                temp_A3 += 2 * np.pi * d2
                                            else:
                                                temp1 = d2 * np.arccos((distance_ij**2 + d2 - d2bis) / (2 * distance_ij * distancek_1))
                                                temp2 = d2bis * np.arccos((distance_ij**2 + d2bis - d2) / (2 * distance_ij * distancek))
                                                temp3 = 0.5 * np.sqrt((- distance_ij + distancek_1 + distancek)
    ...
    def intersection2D_new(self):
        ...
        :return A: np array, A matrix
        """
        n1, n2 = len(self.MPP1_ROI), len(self.MPP2_ROI)
        A = np.zeros((self.N_fit - 1, self.N_fit - 1))
        for r in range(self.N_fit - 1):
            A[r, r] = n1
        S = np.zeros((self.N_fit, self.N_fit))
                            ...
                            for o in range(max(i - 1, 0), min(i + 1, len(self.imgw1) - 1) + 1):
                                for l in range(max(j - 1, 0), min(j + 1, len(self.imgw1[i]) - 1) + 1):
                                    for y2, x2, s2, db2 in self.imgw1[o][l]:
                                        ...
                                        if d > 0:
                                            if m + d < M:
                                                S[k1, k2] = S[k1, k2] + np.pi * m ** 2
                                            else:
                                                if d < (m + M):
                                                    temp1 = m ** 2 * np.arccos((d ** 2 + m ** 2 - M ** 2) / (2 * d * m))
                                                    temp2 = M ** 2 * np.arccos((d ** 2 + M ** 2 - m ** 2) / (2 * d * M))
                                                    temp3 = 0.5 * np.sqrt((-d + m + M) * (d + m - M) * (d - m + M) * (d + m + M))
                                                    S[k1, k2] = S[k1, k2] + temp1 + temp2 - temp3

        for i in range(self.N_fit - 1):
            for j in range(self.N_fit - 1):
                vol = np.pi * (self.distance_fit[j + 1] ** 2 - self.distance_fit[j] ** 2)
    ...
    def reduced_Ripley_vector(self, **kwargs):
        ...
        G0 = np.dot(np.linalg.inv(A_b), G - mean) / np.sqrt(var)
    ...
    def draw_G0(self, G0):
        ...
        :param G0: np array, G0 matrix
        """
        T = np.sqrt(2 * np.log(len(G0)))
    ...
    def main2D_corr(self, G, var, A):
        ...
        n1, n2 = len(self.MPP1_ROI), len(self.MPP2_ROI)
        delta_K = G[0,:]
        sigma_T = np.zeros([self.N_fit - 1])
        Num = delta_K * (n1*n2/self.ROIarea)
        mu_tab = self.mean_G() * (n1*n2/self.ROIarea)
        for p in range(self.N_fit-1):
            if p > 0:
                T_p = (np.sqrt(2 * np.log(p+1)))
            else:
                T_p = np.sqrt(2)
            sigma_T[p] = (n1*n2*T_p/self.ROIarea)*np.sqrt(var[p])

        A_matb = A * 1/n1
        A_matb_inverse = np.linalg.inv(A_matb)
        C = np.dot(A_matb_inverse, (Num - mu_tab))
    ...
    def coupling_prob(self, **kwargs):
        ...
        # T = [np.sqrt(2 * np.log(i+1)) if i > 0 else np.sqrt(2) for i in range(self.N_fit)]
        T = np.sqrt(2 * np.log(len(G0)))
        coupling = np.array([
            (np.sqrt(var[i]) * G0[i]) / G[i] if G0[i] > T else 0 for i in range(len(self.rings)-1)
        ])

        # Create distance matrix and coupling probability matrix using cdist
        MPP1_array = np.ndarray((n1, 2))
        for i, (c1, s1, (y1, x1)) in enumerate(self.MPP1_ROI):
            MPP1_array[i] = np.array([y1, x1])

        MPP2_array = np.ndarray((n2, 2))
        for i, (c1, s1, (y2, x2)) in enumerate(self.MPP2_ROI):
            MPP2_array[i] = np.array([y2, x2])

        dist_array = cdist(MPP1_array, MPP2_array)
        prob_array = np.zeros(dist_array.shape)
        for i in range(len(self.rings) - 1):
            prob_array[np.where(np.logical_and(self.rings[i] <= dist_array, dist_array < self.rings[i+1]))] = coupling[i]
        prob_array[dist_array == 0] = 0

        # Create the list of lines for the couples excel file
        n_couples = np.sum(prob_array > 0)
        prob_write = []
        for i in range(n1):
            for j in range(n2):
                if prob_array[i,j] > 0:
        ...
        if np.sum(prob_array) > 0:
            coupling_index = (np.sum(prob_array)/n1, np.sum(prob_array)/n2)
            mean_coupling_distance = np.sum(dist_array*prob_array)/np.sum(prob_array)
        else:
            coupling_index = (0,0)
    ...
    def beta_correction(step, nbN):
        ...
        alpha = np.zeros(N_h + 1)
        results = np.zeros(N_h + 1)
        ...
        for i in range(results.size):
            ...
            h = alpha[i] + step
            while h <= 1:
                results[i] = results[i] + h * step / (1 - 1 / np.pi * np.arccos(alpha[i] / h))
                h = alpha[i] + j * step
                j += 1
    ...
    def boundary_condition(self, h, x1, x2, y1, y2):
        ...
        :param X: A np 1D array of x random coordinates
        :param Y: A np 1D array of y random coordinates

        :returns : The boundary condition
        """
        d = self.distance(x1, x2, y1, y2)
        #minimum = np.array([min(h, d) for d in dist])
        if d == 0:
            d = 0.0000001
        minimum = min(h, d)

        k = (1 - 1/np.pi * np.arccos(minimum / d))
        return 1 / k

    def mean_G(self):
        """ This function computes the mean of G in 2D
        """
        mean = np.pi * np.diff(np.array(self.distance_fit) ** 2)
        return mean