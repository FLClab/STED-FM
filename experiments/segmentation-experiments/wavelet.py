...
def detect_spots(image: numpy.ndarray) -> numpy.ndarray:
    mask = DetectionWavelets(
        img=image,
        J_list=[3, 4],
        scale_threshold=2.0
    ).computeDetection()
    spots_img = filter_spots(mask)
    return spots_img

def filter_spots(mask: numpy.ndarray) -> numpy.ndarray:
...
class DetectionWavelets:
    ...
    def __init__(self, img, J_list=(3,4), scale_threshold=2.0):
        """Init function
