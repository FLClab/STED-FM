...
def make_plot(results: dict, model: str = "MAE_SMALL", dataset: str = "synaptic-proteins", probe: str = "fine-tuning") -> None:
    ...
    keys = ["1", "10", "25", "50", "100"]
    assert len(keys) == len(sted_data)

    fig = plt.figure()
    plt.plot(keys, sted_data, marker='o', color='tab:blue', label="STED")
    plt.xlabel("Label %")
    plt.ylabel("Accuracy")
...
def main():
    make_plot(results=FEWSHOT, model="MAE_SMALL", dataset="synaptic-proteins", probe='fine-tuning')
