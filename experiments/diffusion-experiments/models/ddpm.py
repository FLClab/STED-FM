...
class DDPM(nn.Module):
    ...
    def p_reconstruct_loop(
        ...
        final = None
        for sample in self.p_reconstruct_loop_progressive(
            x_t=x_t,
            t=t,
            clip_denoised=clip_denoised,
            denoised_fn=denoised_fn,
    ...
    def p_reconstruct_loop_progressive(
        self,
        x_t: torch.Tensor,
        t: int,
        clip_denoised: bool = True,
        ...
        end_t = t[0].item()
        indices = list(range(end_t))[::-1]
        
        if progress: 
            from tqdm.auto import tqdm 
            indices = tqdm(indices, desc="Iterative reconstructing...")
