...
from stedfm.datasets import MICRANetHDF5Dataset, CreateFActinBlockGluGlyDataset
...
parser.add_argument("--boundary", type=str, default="factin-rings-fibers")
...
def main():
    ...
    if args.figure:
        ...
        dataset = MICRANetHDF5Dataset(
            os.path.join(BASE_PATH, "evaluation-data", "actin-data", f"train_01-04-19.hdf5"),
            validation=True,
        ...
        dataset = CreateFActinBlockGluGlyDataset(
            os.path.join(BASE_PATH, "evaluation-data", "actin-data", "EXP192 (18-09-18) - BlockGluGly"),
        ...
        with torch.no_grad():
            for i in tqdm(indices):
                ...
                # Translate condition to target label; this is required since not the same dataset is used.
                condition = {"Block" : "Rings", "GLU-GLY" : "Fibers"}[condition]
