...
def main():
    ...
    for folder in ["CK-666/KD", "Cofilin1KD/KD", "PFN1KO/KO", "TBeta4KD/KD"]:
        files = list(sorted(glob.glob(os.path.join(BASE_PATH, "evaluation-data", "BBBC052", folder, "**/*.tif*"), recursive=True)))
    ...
    with open(os.path.join(BASE_PATH, "evaluation-data", "BBBC052", "BBBC052-training.txt"), "w") as f:
        for file in training_files:
            file = file.replace(BASE_PATH, "")
            if file.startswith("/"):
                file = file[1:]
            f.write(file + "\n")
    with open(os.path.join(BASE_PATH, "evaluation-data", "BBBC052", "BBBC052-validation.txt"), "w") as f:
        for file in validation_files:
            file = file.replace(BASE_PATH, "")
            if file.startswith("/"):
                file = file[1:]
            f.write(file + "\n")
    with open(os.path.join(BASE_PATH, "evaluation-data", "BBBC052", "BBBC052-testing.txt"), "w") as f:
