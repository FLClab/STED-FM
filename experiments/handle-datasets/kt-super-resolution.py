...
OUTPATH = os.path.join(BASE_PATH, "super-resolution-data", "kt-super-resolution")
...
def add_files_to_tar(condition, files, split):
    ...
    with tarfile.open(os.path.join(OUTPATH, f"{split}-dataset.tar"), "a") as handle:
        ...
        for i, f in enumerate(tqdm(files, desc=f"{condition} ({split}) files...")):
            ...
            try:
                bad_data = tifffile.imread(f.replace("Good_STED", "Confocal"))
            except Exception as e:
                print(f"Error reading file {f.replace('Good_STED', 'Confocal')}: {e}")
                continue
