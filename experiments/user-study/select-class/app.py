...
def index():
    ...
    global user, current_idx, template_images, candidate_classes

    if user is None:
        return render_template('login.html')

    # When all the images have been annotated, display the done page
    if current_idx >= len(template_images):
        logger.log(logging.INFO, "User ({}) is done".format(user))
        return redirect(url_for('done'))

    return render_template('index.html', template_image=template_images[current_idx], candidate_classes=candidate_classes)

@app.route('/choose', methods=['POST'])
def choose():

    global user, current_idx, template_images, user_choices
