...
tensorboard --logdir="/scratch/anbil106/projects/SSL/baselines/dataset-fullimages-1Msteps-multigpu/convnext-small_STED" --host 0.0.0.0 --load_fast false &

CKPT="/home/anbil106/scratch/projects/SSL/baselines/dataset-fullimages-1Msteps-multigpu/convnext-small_STED/result.pt"
if [ -f $CKPT ]; then
    echo "% Training from previous checkpoint: ${CKPT}"

    srun python main-lightning.py --seed 42 --use-tensorboard --dataset-path "${SLURM_TMPDIR}/dataset.tar" --backbone "convnext-small" \
                                  --save-folder "./data/SSL/baselines/dataset-fullimages-1Msteps-multigpu" \
                                  --opts "batch_size 16 datamodule.return_metadata False" \
                                  --restore-from "${CKPT}"
else
