...
CKPT="/home/anbil106/scratch/projects/SSL/baselines/dataset-crops-1Msteps-multigpu/convnext-large_STED/result.pt"
if [ -f $CKPT ]; then
    echo "% Training from previous checkpoint: ${CKPT}"

    srun python main-lightning.py --seed 42 --use-tensorboard --dataset-path "${SLURM_TMPDIR}/dataset.tar" --backbone "convnext-large" \
