...
srun --ntasks=$SLURM_NNODES --ntasks-per-node=1 cp "/project/def-flavielc/datasets/FLCDataset/sim-dataset-full-images.tar" "${SLURM_TMPDIR}/dataset.tar"
...
tensorboard --logdir="/scratch/anbil106/projects/SSL/baselines/dataset-fullimages-1Msteps-multigpu/resnet18_SIM" --host 0.0.0.0 --load_fast false &

CKPT="/home/anbil106/scratch/projects/SSL/baselines/dataset-fullimages-1Msteps-multigpu/resnet18_SIM/result.pt"
if [ -f $CKPT ]; then
    ...
    srun python main-lightning.py --seed 42 --use-tensorboard --dataset-path "${SLURM_TMPDIR}/dataset.tar" --backbone "resnet18" \
                                  ...
                                  --opts "batch_size 128 datamodule.return_metadata False" \
                                  --restore-from "${CKPT}"
else
