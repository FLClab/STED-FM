...
python eval.py \
    --restore-from "./data/SSL/segmentation-baselines/mae-small/factin/${RESTOREFROM[${SLURM_ARRAY_TASK_ID}]}/result.pt" \
    --dataset "factin" \
    --opts batch_size 32

python eval-per-percentage.py \
    --restore-from "./data/SSL/segmentation-baselines/mae-small/factin/${RESTOREFROM[${SLURM_ARRAY_TASK_ID}]}/" \
    --dataset "factin" \
    --opts batch_size 32    
