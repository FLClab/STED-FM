...
def get_dataset(name: str, cfg: Configuration, **kwargs) -> Dataset:
    ...
    if name == "jmb-lqhq":
        ...
        testing_dataset = LQHQDenoisingDataset(
            ...
            classes=["VGAT_ATTO490LS", "Gephyrin_STARRED"],
            n_channels=cfg.in_channels, **kwargs)
    elif name == "kt-sr-vgat":
        path = os.path.join(BASE_PATH, "super-resolution-data", "kt-super-resolution")
        training_dataset = LQHQDenoisingDataset(
            tarpath=os.path.join(path, "train-dataset.tar"),
            n_channels=cfg.in_channels, 
            transform=transform,
    ...
    # import tifffile
    # os.makedirs(os.path.join(path, name, "exported", "train", "raw"), exist_ok=True)
    # os.makedirs(os.path.join(path, name, "exported", "train", "gt"), exist_ok=True)

    # os.makedirs(os.path.join(path, name, "exported", "test", "raw"), exist_ok=True)
    # os.makedirs(os.path.join(path, name, "exported", "test", "gt"), exist_ok=True)
    # stack = []
    # for i in range(len(training_dataset)):
    #     images, _ = training_dataset[i]
    #     # images = transforms.functional.resize(images, (256, 256), interpolation=transforms.InterpolationMode.BILINEAR)
    ...
    #     tifffile.imwrite(os.path.join(path, name, "exported", "train", "raw", f"img_{i:04d}.tif"), stack[i, 0, :, :])
    #     tifffile.imwrite(os.path.join(path, name, "exported", "train", "gt", f"img_{i:04d}.tif"), stack[i, 1, :, :])
    # tifffile.imwrite(os.path.join(path, name, "exported", "train", "raw_stack.tif"), stack[:, 0, :, :])
    # tifffile.imwrite(os.path.join(path, name, "exported", "train", "gt_stack.tif"), stack[:, 1, :, :])
    ...
    # stack = numpy.stack(stack, axis=0)
    # for i in range(stack.shape[0]):
    #     tifffile.imwrite(os.path.join(path, name, "exported", "test", "raw", f"img_{i:04d}.tif"), stack[i, 0, :, :])
    #     tifffile.imwrite(os.path.join(path, name, "exported", "test", "gt", f"img_{i:04d}.tif"), stack[i, 1, :, :])
    # tifffile.imwrite(os.path.join(path, name, "exported", "test", "raw_stack.tif"), stack[:, 0, :, :])
    # tifffile.imwrite(os.path.join(path, name, "exported", "test", "gt_stack.tif"), stack[:, 1, :, :])
    # exit()
