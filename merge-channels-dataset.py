...
def find_matches(info, metadata):
    matches = []
    for name, candidate in metadata.items():
        if candidate["image-id"] == info["image-id"]:
...
def main():
    ...
    BASEPATH = "/home-local2/projects/FLCDataset"
    OUTPATH = "/home-local2/projects/FLCDataset/STED-FM-dataset-full-images-multichannel.tar"
    ...
    for key, info in metadata.items():
        matches = find_matches(info, metadata)

        k = info["image-id"]
        if k not in matched_metadata:
            matched_metadata[k] = matches
        
        i+=1
        if i % 100 == 0:
            print("Processed images:", i)

    print("Total unique images with matches:", len(matched_metadata))
    ...
    with tarfile.open(OUTPATH, "a") as tf:
        for key, info in tqdm(metadata.items(), desc="Images"):
            ...
            # Info is a list of matches
            images = [
                tifffile.imread(os.path.join(BASEPATH, match["path"]))
                for match in info
            ]
            ensure_all_shapes = all([images[0].shape == img.shape for img in images])
            if not ensure_all_shapes:
                logging.info("ImageShapeMismatchError")
                logging.info("Not all channels have the same shape... Skipping")
                logging.info(f"{[img.shape for img in images]=}")
                logging.info(f"{info=}")
                continue
            image = numpy.stack(images, axis=0)

            info = info[0]  # Take metadata from the first match
